...
<td width="60%"> <form id='myform' method='POST' action='#'>
    ...
    <input type='text' id="qty1" name='quantity' value='0' class='qty' />
...
<input type="submit" value="add" onClick="return chk3()">
...
<script>
 function chk3(){
	 ...
	 var input = parseInt(document.getElementById('qty1').value);
	 var mat_code1 = '<?php echo $sid ?>';
	 var mat_desc1 = '<?php echo $desc ?>';
	 var mat_price1 = '<?php echo $price ?>';
	 var prev_mat_code = '<?php echo $prev_mat_code ?>';
	if(prev_mat_code != mat_code1){
	 if(input > stock){
	 ...
	 $.ajax({
		type: "post",
		 url:"test2.php",
		 data:{id1:parseInt(document.getElementById('qty1').value),id2: mat_code1,id3: mat_desc1,id4: mat_price1},
		 cache:false,
		 success: function(html){
			 $('#msg3').html(html);
			 }

	 ...
	 if(iCustomerId == ""){
		...
		// type: "post",
		 ...
		 success: function(html){
			 ...
			 $.ajax({
		// type: "post",
		 url:"entry5.php?frm_action&cust_id="+iCustomerId,
		 //data:dataString,
		 cache:false,
		 success: function(html){
			 $('#msg1').html(html);
...
<td width="60%"> <form id='myform' method='POST' action='#'>
    ...
    <input type='text' id="qnt3" name='quantity' value='0' class='qty' />
...
function chk1(){
	 ...
	 var mat_code3 = '<?php echo $sid ?>';
	 var mat_desc3 = '<?php echo $desc ?>';
	 var mat_price3 = '<?php echo $price ?>';
	 var prev_mat_code = '<?php echo $prev_mat_code ?>';
	 var input = parseInt(document.getElementById('qnt3').value);
	if(prev_mat_code != mat_code3){
	 if(input > stock){
	...
	window.scrollTo(0, 300);	
	 $.ajax({
		 type: "post",
		 url:"test4.php",
		 data:{ id1: parseInt(document.getElementById('qnt3').value), id2: mat_code3, id3: mat_desc3, id4: mat_price3},
		 cache:false,
		 success: function(html){
			 $('#msg5').html(html);
			 }

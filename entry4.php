	 ...
	 if(iCustomerId == ""){
		...
		// type: "post",
		 ...
		 success: function(html){
			 ...
			 $.ajax({
		// type: "post",
		 url:"entry4.php?frm_action&cust_id="+iCustomerId,
		 //data:dataString,
		 cache:false,
		 success: function(html){
			 $('#msg1').html(html);
...
<td align="left"><select name="sid" id="sid" onchange="changeSID();">
...
<td width="60%"> <form id='myform' method='POST' action='#'>
    ...
    <input type='text' id="qnt2" name='quantity' value='0' class='qty' />
...
<input type="submit" value="add" onClick="return chk4()">
...
<?php
$result19 = mysqli_query($con,"select mat_code from current_completeorder");
while ($row19 = mysqli_fetch_array($result19)){
	if($sid == $row19['mat_code']){
		$prev_mat_code = $row19['mat_code'];
		break;
	}	
	$prev_mat_code = $row19['mat_code'];
}
...
<script>
 function chk4(){
	 ...
	 var input = parseInt(document.getElementById('qnt2').value);
	 var mat_code2 = '<?php echo $sid ?>';
	 var mat_desc2 = '<?php echo $desc ?>';
	 var mat_price2 = '<?php echo $price ?>';
	 var prev_mat_code = '<?php echo $prev_mat_code ?>';
	if(prev_mat_code != mat_code2){
	 if(input > stock){
	 ...
	 $.ajax({
		type: "post",
		 url:"test3.php",
		 data:{ id1: parseInt(document.getElementById('qnt2').value), id2: mat_code2, id3: mat_desc2, id4: mat_price2},
		 cache:false,
		 success: function(html){
			 $('#msg4').html(html);
			 }
		...
		window.alert("Item already selected please choose another item");
		}
	 return false;
 }
</script>
